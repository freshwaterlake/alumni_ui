            ...
            <div className="d-flex flex-wrap">
                {range(0, control.controlGroup.length - 1).map((index) => {
                    if (index === control.controlGroup.length - 3)
                        return getControlFromFactory(control.controlGroup[index], dataKey, dataKey, state as State, handleLocalStateUpdate);

                    if (index === control.controlGroup.length - 2)
                        return getControlFromFactory(control.controlGroup[index], dataKey, dataKey, state as State, handleItemAdd);
                })}
            </div>
